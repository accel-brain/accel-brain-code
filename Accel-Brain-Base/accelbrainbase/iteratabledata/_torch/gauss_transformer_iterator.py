...
class GaussTransformerIterator(_TransformerIterator):
    ...
    def generate_learned_samples(self):
        ...
        for _ in range(self.iter_n):
            ...
            training_encoded_observed_arr = torch.normal(
                mean=self.__loc,
                std=self.__std,
                size=(
                    self.batch_size,
            ...
            training_encoded_observed_arr = training_encoded_observed_arr.to(self.__ctx).float()

            training_encoded_mask_arr = None

            training_decoded_observed_arr = torch.normal(
                mean=self.__loc,
                std=self.__std,
                size=(
                    self.batch_size,
            ...
            training_decoded_observed_arr = training_decoded_observed_arr.to(self.__ctx).float()

            training_decoded_mask_arr = None

            test_encoded_observed_arr = torch.normal(
                mean=self.__loc,
                std=self.__std,
                size=(
                    self.batch_size,
            ...
            test_encoded_observed_arr = test_encoded_observed_arr.to(self.__ctx).float()

            test_encoded_mask_arr = None
            test_decoded_observed_arr = torch.normal(
                mean=self.__loc,
                std=self.__std,
                size=(
                    self.batch_size,
            ...
            test_decoded_observed_arr = test_decoded_observed_arr.to(self.__ctx).float()
