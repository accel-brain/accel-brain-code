...
class RTRBMGraph(Synapse):
    '''
    Complete Bipartite Graph for RTRBM.
    ...
    # bias in visible layer in `t-1`.
    __pre_visible_bias_arr = None

    def get_pre_visible_bias_arr(self):
        ''' getter '''
        if isinstance(self.__pre_visible_bias_arr, np.ndarray) is False:
            raise TypeError()
        return self.__pre_visible_bias_arr

    def set_pre_visible_bias_arr(self, value):
        ''' setter '''
        if isinstance(value, np.ndarray) is False:
            raise TypeError()
        self.__pre_visible_bias_arr = value

    pre_visible_bias_arr = property(get_pre_visible_bias_arr, set_pre_visible_bias_arr)

    # bias in hidden layer in `t-1`.
    __pre_hidden_bias_arr = None
    
    def get_pre_hidden_bias_arr(self):
        ''' getter '''
        if isinstance(self.__pre_hidden_bias_arr, np.ndarray) is False:
            raise TypeError()
        return self.__pre_hidden_bias_arr

    def set_pre_hidden_bias_arr(self, value):
        ''' setter '''
        if isinstance(value, np.ndarray) is False:
            raise TypeError()
        self.__pre_hidden_bias_arr = value

    pre_hidden_bias_arr = property(get_pre_hidden_bias_arr, set_pre_hidden_bias_arr)

    # Weight matrix of bias in visible layer.
    __visible_bias_weights_arr = None

    def get_visible_bias_weights_arr(self):
        ''' getter '''
        if isinstance(self.__visible_bias_weights_arr, np.ndarray) is False:
            raise TypeError()
        return self.__visible_bias_weights_arr

    def set_visible_bias_weights_arr(self, value):
        ''' setter '''
        if isinstance(value, np.ndarray) is False:
            raise TypeError()
        self.__visible_bias_weights_arr = value

    visible_bias_weights_arr = property(get_visible_bias_weights_arr, set_visible_bias_weights_arr)

    # Weights matrix of bias in hidden layer.
    __hidden_bias_weights_arr = None

    def get_hidden_bias_weights_arr(self):
        ''' getter '''
        if isinstance(self.__hidden_bias_weights_arr, np.ndarray) is False:
            raise TypeError()
        return self.__hidden_bias_weights_arr

    def set_hidden_bias_weights_arr(self, value):
        ''' setter '''
        if isinstance(value, np.ndarray) is False:
            raise TypeError()
        self.__hidden_bias_weights_arr = value

    hidden_bias_weights_arr = property(get_hidden_bias_weights_arr, set_hidden_bias_weights_arr)

    # Acitivity in hidden layer in `t-1`.
    ...
    # Activation function for RNN hidden units.
    __rnn_activating_function_interface = None

    def get_rnn_activating_function_interface(self):
        ''' getter '''
        if isinstance(self.__rnn_activating_function_interface, ActivatingFunctionInterface) is False:
            raise TypeError()
        return self.__rnn_activating_function_interface

    def set_rnn_activating_function_interface(self, value):
        ''' setter '''
        if isinstance(value, ActivatingFunctionInterface) is False:
            raise TypeError()
        self.__rnn_activating_function_interface = value

    rnn_activating_function_interface = property(get_rnn_activating_function_interface, set_rnn_activating_function_interface)
    ...
    def create_node(
        ...
        self.pre_visible_bias_arr = np.zeros(self.visible_bias_arr.shape)
        self.pre_hidden_bias_arr = np.zeros(self.hidden_bias_arr.shape)

        self.visible_bias_weights_arr = np.zeros((
            self.visible_bias_arr.shape[0],
            self.hidden_bias_arr.shape[0]
        ))
        self.hidden_bias_weights_arr = np.zeros((
            self.hidden_bias_arr.shape[0],
            self.visible_bias_arr.shape[0]
        ))
        
        self.pre_hidden_activity_arr = np.zeros(self.hidden_bias_arr.shape)
