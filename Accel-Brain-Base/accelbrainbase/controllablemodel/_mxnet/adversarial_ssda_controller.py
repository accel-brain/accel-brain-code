...
class AdversarialSSDAController(HybridBlock, ControllableModel):
    '''
    Adversarial Self-supervised domain adaptation.
    ...
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(adversarial_ssda_loss, AdversarialSSDALoss) is False:
            raise TypeError("The type of `adversarial_ssda_loss` must be `AdversarialSSDALoss`.")
        ...
        with self.name_scope():
            ...
            self.register_child(self.discriminator)

        self.adversarial_ssda_loss = adversarial_ssda_loss
    ...
    def collect_params(self, select=None):
        ...
        params_dict.update(self.discriminator.collect_params(select))
        ...
        try:
            ...
            for batch_observed_arr, batch_target_arr, test_batch_observed_arr, test_batch_target_arr, pretext_arr, pretext_label_arr in iteratable_data.generate_learned_samples():
                ...
                with autograd.record():
                    ...
                    source_posterior_arr = self.discriminator.inference(source_encoded_arr)
                    target_posterior_arr = self.discriminator.inference(target_encoded_arr)
                    ...
                    loss, classification_loss, pretext_loss, discriminator_loss = self.compute_loss(
                        pretext_pred_arr, 
                        pred_arr, 
                        pretext_label_arr, 
                        batch_target_arr, 
                ...
                if est_flag is False:
                    ...
                    if (iter_n+1) % int(iteratable_data.iter_n / iteratable_data.epochs) == 0:
                        ...
                        test_loss, test_classification_loss, test_pretext_loss, test_discriminator_loss = self.compute_loss(
                            pretext_pred_arr, 
                            pred_arr, 
                            pretext_label_arr, 
                            batch_target_arr, 
                        ...
                        self.__logger.debug("discriminator's loss in train: " + str(discriminator_loss.asnumpy().mean()) + " discriminator's loss in test: " + str(test_discriminator_loss.asnumpy().mean()))
                    ...
                    if (iter_n+1) % int(iteratable_data.iter_n / iteratable_data.epochs) == 0:
                        self.__loss_list.append(
                            (
                                ...
                                discriminator_loss.asnumpy().mean(),
                                test_discriminator_loss.asnumpy().mean()
                            )
                        )
                        if self.compute_acc_flag is True:
    ...
    def compute_loss(
        ...
        Returns:
            Tensor of losses.
        '''
        return self.adversarial_ssda_loss(
            pretext_pred_arr, 
            pred_arr, 
            pretext_label_arr, 
            label_arr, 
